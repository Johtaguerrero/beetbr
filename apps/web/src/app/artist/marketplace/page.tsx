...
function ListingCard({ listing, saved, onSave }: { listing: Listing; saved: boolean; onSave: () => void }) {
    ...
    return (
        ...
        >
            ...
            <Link href={`/artist/marketplace/listing/${listing.id}`} className="flex flex-col flex-1 px-4 pb-4 gap-3" style={{ textDecoration: 'none' }}>
...
export default function MarketplaceHome() {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (selectedCat || search) router.push(`/artist/marketplace?q=${encodeURIComponent(search)}&c=${selectedCat}`);
    };
                            ...
                            return (
                                <div key={section.title}>
                                    <div className="flex items-center justify-between mb-5">
                                        ...
                                        <Link href={`/artist/marketplace`} className="text-[10px] font-bold uppercase tracking-widest text-muted hover:text-accent transition-colors">Ver todos →</Link>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {items.map((l) => (
                                            <ListingCard key={l.id} listing={l}
