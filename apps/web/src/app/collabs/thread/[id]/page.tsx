...
export default function CollabThreadPage() {
    ...
    useEffect(() => {
        if (params.id) {
            fetchThreadMessages(params.id as string);
        }
    }, [params.id, fetchThreadMessages]);
    ...
    const otherScore = otherParticipant?.score || 0;
            ...
            <div className="flex flex-col relative overflow-hidden w-full md:max-w-[480px] h-[calc(100dvh-64px-80px)] md:h-[calc(100vh-6rem)] md:rounded-2xl md:border" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-nav-border)' }}>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-beet-green uppercase font-black tracking-widest">{thread.collab?.title || 'Collab'}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-white/10">
                    <div className="flex flex-col items-center justify-center py-8 text-center opacity-40">
                        ...
                        <p className="text-[10px] max-w-[200px]">Vocês agora estão conectados para colaborar em: <span className="text-white font-bold">"{thread.collab?.title}"</span></p>
                    </div>
